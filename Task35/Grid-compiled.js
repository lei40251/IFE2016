    ...
    Table.prototype = {
        ...
        init: function init() {
            ...
            for (var i = 0; i < this.config.amount; ++i) {
                ...
                for (var j = 0; j < this.config.amount; ++j) {
                    if (i == 0) {
        ...
        _createEle: function _createEle(tag, className, x_axis, y_axis) {
        ...
        _createRobot: function _createRobot(x_axis, y_axis) {
            var robot = this._createEle('div', 'robot', x_axis, y_axis);
        ...
        robotGo: function robotGo(option) {
            var defaultOpt = {
                direction: 0,
                num: 1
            };
            var robot = document.querySelector('.robot');
            var deg = /[+-]?[\d]+/.exec(robot.style.transform);
        ...
        robotTurn: function robotTurn(command) {
            var robot = document.querySelector('.robot');
            var deg = /[+-]?[\d]+/.exec(robot.style.transform);
            switch (command) {
                case 'TUN LEF':
